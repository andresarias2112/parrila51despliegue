...
def registrar_pedido():
    ...
    for p in productos_raw:
        productos.append({
            'id_producto': p[0] if not isinstance(p, dict) else p.get('id_producto'),
            'nombre': p[1] if not isinstance(p, dict) else p.get('nombre'),
            'precio': p[2] if not isinstance(p, dict) else p.get('precio'),
            'id_categoria': p[3] if not isinstance(p, dict) else p.get('id_categoria'),
            'estado': p[4] if not isinstance(p, dict) else p.get('estado', 'Disponible'),
            'cantidad': p[5] if not isinstance(p, dict) else p.get('cantidad', 0),
            'nombre_categoria': p[6] if not isinstance(p, dict) else p.get('nombre_categoria', '')
        })
...
def historial_ordenes_empleado():
    ...
    return render_template('historial_ordenes_em.html', 
                         ordenes=ordenes, 
                         search_query=search_query,
                         page=page,
                         total_pages=total_pages)
